...
gnm <- function(formula, eliminate = NULL, ofInterest = NULL,
    ...
    argPos <- match(
        c(
            "eliminate", "data", "subset", "weights", "na.action",
    ...
    if (is.empty.model(modelTerms) && is.null(eliminate)) {
        ...
        modelAIC <- suppressWarnings(family$aic(
            y, rep.int(1, nobs), mu,
            weights, dev
        ))
        fit <- list(
            coefficients = numeric(0), constrain = numeric(0),
        ...
        if (is.null(start)) {
            ...
            if (!is.null(eliminate) && length(start) == nParam) {
                ...
                stop(
                    "length(start) must either equal the no. of parameters\n",
        ...
        if (onlyLin) {
            ...
            fit <- do.call(method, list(
                modelTools = modelTools, y = y,
                constrain = constrain,
            ...
            fit <- gnmFit(
                modelTools = modelTools, y = y, constrain = constrain,
    ...
    if (identical(ofInterest, "[?]")) {
        call$ofInterest <- ofInterest <-
            pickCoef(fit,
                warningText = paste(
                    "No subset of coefficients selected",
    ...
    if (!is.null(ofInterest)) {
        if (!all(ofInterest %in% seq_len(nParam))) {
            stop(
                "'ofInterest' does not specify a subset of the ",
    ...
    asY <- c(
        "predictors", "fitted.values", "residuals", "prior.weights",
        "weights", "y", "offset"
    )
