...
singular vectors. The plot represents the levels of the two classifying factors by
...
To illustrate the use of biplot models, we shall use the \texttt{barley} data
...
@ %def 
using the \texttt{wedderburn} family function introduced in Section
...
$A$ and $B$ is unaffected by rotation or reciprocal scaling along either axis,
...
relationship between the response and the covariates in the model, but the scale of
this association varies between categories and there is an additional category main
effect or category-specific intercept:
...
using a \texttt{gnm} model with a log link and poisson variance function.

For example, the \texttt{backPain} data set from \citet{Ande84} describes the
...
oneDimensional
@ %def 
using eliminate to handle the \texttt{.rowID} so these structural parameters do
...
@ %def 
which show that the oneDimensional model is perfectly adequate.
...
\begin{tabular*}{\textwidth}{@{}p{0.2in}p{1in}p{4in}@{}}
    ...
    \midrule
    &	gnm	&  fit generalised nonlinear models	\\
    &	gnmControl	&  set control parameters for fitting \texttt{gnm} models	\\
    &	MPinv	&  get Moore-Penrose pseudoinverse of a real-valued matrix	\\
    \midrule
    ...
    &	Mult	&  specify a multiplicative interaction in a \texttt{gnm} formula	\\
    &	Exp	&  specify an exponentiated factor in a \texttt{Mult} term	\\
    &	Nonlin	&  specify a special nonlinear term in a \texttt{gnm} formula	\\
    &	Dref	&  ``plug-in'' function to fit diagonal reference terms	\\
    &	MultHomog	&  ``plug-in'' function to fit multiplicative interactions with homogenous effects	\\
    &   Wedderburn      &  specify the Wedderburn quasi-variance family \\ 
    \midrule
    \multicolumn{3}{l}{\textbf{Methods and Accessor Functions}}			\\
    \midrule
    &	summary.gnm	&  summarize \texttt{gnm} fits	\\
    &	getContrasts	&  estimate contrasts and their standard errors	\\
    &	checkEstimable	&  check whether one or more parameter combinations in a
    \texttt{gnm} model is identified \\
    &	se	&  get standard errors of linear parameter combinations in
    \texttt{gnm} models	\\
    &	termPredictors	&  (generic) extract term contributions to predictor	\\
    \midrule
    \multicolumn{3}{l}{\textbf{Auxiliary Function}}			\\
    \midrule
    &	getModelFrame	&  get the model frame in use by \texttt{gnm}	\\
\end{tabular*}
\end{table}
