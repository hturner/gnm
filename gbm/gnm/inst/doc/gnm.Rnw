...
applications, and Section \ref{Stereotype} considers the stereotype model, which
...
\[\mu_{ijkl} = \alpha_{il} + \beta_{jkl} + \delta_l\gamma_{ij} + \phi_l\theta_{ik},\]
where 
The \texttt{cautres} data set can be used to illustrate the application of
UNIDIFF models to four-way contingency tables. This data set is classified by
the variables vote, class, religion and election.

\subsection{Generalized Additive and Multiplicative (GAMMI) Models}
\label{GAMMI}
