...
<<>>=
getContrasts(RChomog,  grep("Mult", names(coef(RChomog))))
@ 

<<>>=
multConstrained <- getContrasts(RChomog,  
                                grep("Mult", names(coef(RChomog))))[[1]][[1]]
diagonal <- coef(RChomog)[grep("Diag", names(coef(RChomog)))]
set.seed(1)
RChomogConstrained1 <- update(RChomog, constrain = 31, 
                             start = c(rep(NA, 15), diagonal, multConstrained))
set.seed(2)
