...
\usepackage[left=2cm,nohead]{geometry}
%\usepackage{hyperref}
...
model-fitting functions.  These are: two new data-manipulation functions
\Rfunction{Diag}
and \Rfunction{Symm}, for setting up structured interactions between
homologous factors; a new \Rclass{family} function, \Rfunction{wedderburn},
...
multiplicative component unless otherwise specified. For example,
...
multiplicative components so that the component is always nonnegative.
This may be
achieved by specifying the component as an exponential, as in the
following `uniform difference' model \citep{Xie92, Erik92}
\[
\log \mu_{rct} = \alpha_{rt} + \beta_{ct} + e^{\gamma_t}\delta_{rc}.
\]
Exponentiated components are specified in \Rclass{gnm} models using the
symbolic
...
which requires a plug-in function to estimate the associated parameters. There
are two arguments to \Rfunction{Nonlin}: a call to the relevant plug-in function
and if necessary, a \Rclass{data.frame} object containing any variables that are
required uniquely by specified arguments of the plug-in function.
...
plug-in function
\Robject{PlugInFunction}. As the factor $C$ only appears in the specified
arguments of the
call to \Robject{PlugInFunction}, a data frame containing factor
\Robject{C} has been passed as the
\Rfunarg{data} argument of \Rfunction{Nonlin}.
Note that this would not be necessary
if \Robject{C} could be found in an environment on the search path (given by
\texttt{search()}), however \Robject{C} would only appear in the returned
model frame if it was passed directly to \Rfunction{Nonlin}.
...
in which the level effects are constrained to be
equal across the multiplicative components.
The arguments of \Rfunction{MultHomog}
are the factors in
the interaction, which are assumed to be objects of class \Rclass{factor}.
Like a
\Rfunction{Mult} term, the interaction can include any number of multiplicative
components, but there is no multiplicity argument.
...
description of the dependence of $\delta_i$ on any covariates.
...
those that are directly estimated, i.e. the $\delta_i$ or the $\xi_i$ and
$\beta_i$, rather than the implied weights $w_i$.
...
environment on the search path, since \Rfunction{gnm} does not assume this.
Rather,
the function \Rfunction{getModelFrame} should be used to get the
model frame used
by \Rfunction{gnm}, which will have all the model variables and also attributes
useful for \Rfunction{model.matrix} etc.

For example, the first few lines of the \Rfunction{MultHomog} function are
\begin{alltt}
    MultHomog <- function(...)\{
    ...
    ...
\end{alltt}
The names of the factors in the interaction are assigned to \Robject{labelList},
and the model frame used by \Rfunction{gnm} is assigned to \Robject{gnmData}.
The
factors can then be accessed by name from \Robject{gnmData}, as in the call to
\Rfunction{lapply}.
...
\begin{alltt}
    ...
    \}
\end{alltt}
This function only requires the argument \Rfunarg{coef}, but since the
...
\item
Compute the \Rfunction{glm} estimate of any parameters in linear terms that were
not specified by \Rfunarg{start},
offsetting the contribution to the predictor of
any parameters specified by \Rfunarg{start} or a plug-in function.
\item
Run starting iterations: update any parameters in nonlinear terms
that were not specified by \Rfunarg{start} or a plug-in function one at a time,
updating \emph{all} linear terms after each round of nonlinear updates.
\end{enumerate}
Note that no starting iterations (step 5) will be run if all
parameters are specified by the \Rfunarg{start} argument of \Rfunction{gnm}.
...
will return a random parameterization.
...
parameters are excluded from the returned
vector of coefficients, so that summaries of the model focus on the
...
\Rpackage{gnm} package provides a new generic function
...
doubleUnidiff <- gnm(Freq ~ election:vote + election:class:religion
                     ...
                     data = cautres)
@
The effects of the first multiplicative component of the first multiplicative
term are
...
@
and then their estimability can be checked using \Rfunction{checkEstimable}
...
However, applying the equivalent constraint to the second component in the
interaction is not sufficient to make the parameters in that component
estimable:
...
standard errors. Returning to the example of the first component in the first
...
(religion-vote association) multiplicative component produces the
following result:
<<Get_contrasts_2>>=
...
The RC(1) model can be extended to an RC($m$) model with $m$ instances of the
...
@ %def 
This model is slightly different from that reported by \citet{Clif93}, which we
believe may be due to a transcription error in the published data. 
...
\begin{equation*}
\mu_{od} = \begin{cases}
\frac{e^{\delta_1}}{e^{\delta_1} + e^{\delta_2}}\gamma_o +
\frac{e^{\delta_2}}{e^{\delta_1} + e^{\delta_2}}\gamma_d & \text{if } i = 1\\
\frac{e^{\delta_3}}{e^{\delta_3} + e^{\delta_4}}\gamma_o +
\frac{e^{\delta_4}}{e^{\delta_3} + e^{\delta_4}}\gamma_d & \text{if } j = 1\\
\frac{e^{\delta_5}}{e^{\delta_5} + e^{\delta_6}}\gamma_o +
\frac{e^{\delta_6}}{e^{\delta_5} + e^{\delta_6}}\gamma_d & \text{if } i \ne 1
\text{ and } j \ne 1
\end{cases}
\end{equation*}
...
\citet{Vand02}. The data from this paper are not publically available, but we
...
This model can be applied to the \Robject{yaish} data set, which is a
...
The \Robject{cautres} data set can be used to illustrate the application of the
...
and column main effects plus one or more terms of  multiplicative
interaction. The singular value corresponding to each  multiplicative term
is often
factored out, as a measure of the strength of association between the row and
column scores, indicating the importance of the component, or axis.
...
multiplicative term $k$ and $\sigma_k$ is the singular value for term $k$. The
number of multiplicative terms, $K$,
is less than or equal to the rank of the matrix of
...
A rank-$n$ model is a model based on the first $n$ terms of the
decomposition. In the case of a singular value decomposition, this is equivalent
to a model with $n$ instances of the multiplicative interaction.
...
so we can rotate the data so that the points for the sites are roughly parellel
...
\begin{table}[!h]
\begin{tabular*}{\textwidth}{@{}p{0.2in}p{1.3in}p{4in}@{}}
    \toprule
    \multicolumn{3}{l}{\textbf{Model Fitting}} 	\\
    \midrule
    &	\Rfunction{gnm}	&  fit generalised nonlinear models	\\
    ...
    &	\Rfunction{Exp}	&  specify an exponentiated factor in a
                           \Rfunction{Mult} term	\\
    &	\Rfunction{Nonlin}	&  specify a special nonlinear term in a
                                   \Rfunction{gnm} formula	\\
    &	\Rfunction{Dref}	&  ``plug-in'' function to fit diagonal
                                   reference terms \\
    &	\Rfunction{MultHomog}	&  ``plug-in'' function to fit multiplicative
                                   interactions with homogenous effects	\\
