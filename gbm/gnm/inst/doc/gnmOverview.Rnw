...
\begin{figure}
    \begin{center}
        <<residSVDplot, fig = TRUE, echo = FALSE>>= plot(coef(biplotModel),
        biplotStart, main = "Comparison of residSVD and MLE for a
        2-dimensional biplot model") @ %def
    \end{center}
...
<<Rotate_and_scale,include = FALSE>>=
a <- pi/5
