...
The \Rfunction{Mult} function has one specified argument
\Rfunarg{multiplicity}, which is
...
The function \Rfunction{Dref} is a plug-in  to fit diagonal reference terms
involving
...
The \Rfunction{Dref} function has one specified argument, \Rfunarg{formula},
which is a symbolic
...
\Rcode{Variables}\null. For example, the (non-visible) companion function for
