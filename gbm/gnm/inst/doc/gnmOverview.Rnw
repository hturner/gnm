...
\date{For \Rpackage{gnm} version 0.60-1, 2005-06-24}
...
equal across the constituent multipliers.
The arguments of \Rfunction{MultHomog}
are the factors in
the interaction, which are assumed to be objects of class \Rclass{factor}.
Like a
\Rfunction{Mult} term, the interaction can include any number of constituent
multipliers, but there is no multiplicity argument.
...
\begin{description}
\item[\Robject{start}]
  either a vector of default starting values
...
doubleUnidiff <- gnm(Freq ~ election:vote + election:class:religion
                     ...
                     data = cautres)
@
The effects of the first constituent multiplier of the first multiplicative
term are
...
(religion-vote association) constituent multiplier produces the
following result:
<<Get_contrasts_2>>=
