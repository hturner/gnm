...
eliminated parameters are excluded from summaries of the model so that the
focus is on the coefficients of interest.
...
the number of eliminated parameters (here 500).  The eliminated parameters do
...
contrCoefs <- coefs[grep("religion.*\\.1", coefs)]
...
coefs <- names(coef(doubleUnidiff))
contrCoefs <- coefs[grep("religion.*\\.2", coefs)]
nContr <- length(contrCoefs)
...
<<Get_contrasts_1>>=
coefs.of.interest <- grep("religion.*\\.1", names(coef(doubleUnidiff)))
...
\begin{center}
<<qvplot, fig = TRUE>>=
plot(myContrasts[[1]], levelNames = paste("election", c(1,2,3,4)),
...
%following result:
%<<Get_contrasts_2>>=
%coefs.of.interest <- grep("religion.*\\.2", names(coef(doubleUnidiff)))
...
biplotModel <- gnm(y ~ -1 + instances Mult(-1 + site, -1 + variety), 2), 
...
coefs.of.interest <- grep("\\.1", names(coef(unidiff)))
...
getContrasts(doubleUnidiff,
             rev(grep("religion.*\\.1", names(coef(doubleUnidiff)))))
getContrasts(doubleUnidiff,
             rev(grep("class.*\\.1", names(coef(doubleUnidiff)))))
@ %def
...
for the vector of parameters $(\alpha, \beta, \gamma)$.  The median deviance
(in this case, residual sum of squares) among the 100 fits is
essentially zero: in fact, 56 of these 100 fits reproduce the data exactly, to
machine accuracy.
The remaining 44 fits are all identical to one another, but they are far from
...
level equal to $\bar{y}=0.09508$.  For example, the very first of the 100 fits
is of this kind:
@
<<singleExp2>>=
saved.fits[[1]]
...
set.seed(1)
saved.fits <- list()
for (i in 1:100) saved.fits[[i]] <- gnm(y ~ instances(Exp(x), 2), verbose = FALSE)
unlist(sapply(saved.fits, deviance))
@ %def
In this instance, only 13 of the 100 calls to \Rfunction{gnm} have successfully
located a local maximum of the likelihood: in the remaining 87 cases the
...
the last of these, the exact fit to the data, having been found 8 times
out of the above 13).
...
@
<<doubleExp3>>=
gnm(y ~ instances(Exp(x), 2), start = c(NA, NA, -0.1, NA, -0.1), verbose = FALSE)
@ %def
