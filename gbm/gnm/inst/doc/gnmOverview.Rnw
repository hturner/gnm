...
\Rfunction{gnm} function and are specified in the model formula using the
symbolic
function \Rfunction{Mult}.  Other nonlinear terms can be estimated using
plug-in
...
\Rfunction{Dref} has one specified argument, \Rfunarg{formula},
which is a symbolic
...
or through existing plug-in functions provided by the \Rpackage{gnm} package.
...
vector, the ratio is less than \texttt{tolerance\^{}2},
...
indices, a character vector of names, or, if \texttt{constrain = "[?]"} they can
...
If a model contains a factor with hundreds of levels, the standard estimation
procedure is not the best way to estimate the levels of that factor. The
\Rfunarg{eliminate} argument of \Rfunction{gnm} can be used to specify such a
factor in a model, so that this factor can be handled more efficiently. The
factor should be specified by an expression, which is 
...
substantially so if the number of eliminated parameters is large. Second, unless
otherwise specified through the \Rfunarg{ofInterest} argument to
\Rfunction{gnm}, the \Robject{ofInterest} component of the returned model object
indexes the non-eliminated parameters. Thus eliminated parameters are excluded
from printed model summaries and default slection by \Rclass{gnm} methods. See
...
Here we give an artificial illustration: 500 randomly-generated trinomial
...
the number of eliminated parameters (here 500).  Since the default behaviour has
not been overrided by an \Rfunarg{ofInterest} argument, the eliminated parameters do
...
details of all 505 parameters!), which easily can be obtained, if required,
by using \Rfunction{getContrasts}:
\begin{verbatim}
> getContrasts(temp.gnm, ofInterest(temp.gnm)[5:3])
[[1]]
           estimate         SE    quasiSE     quasiVar
resp3:x  0.00000000 0.00000000 0.03169678 0.0010046856
resp2:x -0.06292256 0.05462786 0.04449177 0.0019795175
resp1:x -0.05464657 0.03580324 0.01664891 0.0002771862
\end{verbatim}
...
unidiff <- gnm(Freq ~ educ*orig + educ*dest +
               ...
               constrain = "Mult.*educ1", family = poisson, data = yaish, 
               subset = (dest != 7))      
...
in \Robject{Mult1.Factor1.educ2}, assymetric in \Robject{Mult1.Factor1.educ3}
and \Robject{Mult1.Factor1.educ4} and asymptotic in
\Robject{Mult1.Factor1.educ5}. When the deviance is approximately quadratic in a
given parameter, \Rmethod{profile.gnm} uses the same stepsize for profiling
above and below the original estimate:

\begin{verbatim}
> diff(prof[[2]]$par.vals[, "Mult1.Factor1.educ2"])
...
stepsizes to accomadate the skew:
\begin{verbatim}
> diff(prof[[4]]$par.vals[, "Mult1.Factor1.educ4"])
...
parameters, yet only a subset of these parameters be of interest when it
comes to interpreting the model. An example of this has been seen in
Section~\ref{eliminate}, where a factor is required in the model to represent
a structural aspect of the data and the estimated factor effects have no
substantive interpretation. Yet even for models in which all parameters
correspond to variables of interest, the focus may still be on a subset of
parameters, typically in higher order terms.
...
may not be appropriate, provide a Tk dialog for selection from the parameters of
interest. Parameters may be specified to the \Rfunarg{ofInterest} argument by a
regular expression to match against parameter names, a numeric vector of
indices, a character vector of names, or, if \texttt{ofInterest = "[?]"} they can
...
regular expression against the coefficient names. Otherwise, coefficients may be
selected via a Tk dialog. 
...
myContrasts <- getContrasts(doubleUnidiff, 
                            pickCoef(doubleUnidiff, "Mult1.Factor1"))
myContrasts
@ %def
...
\begin{center}
<<qvplot, fig = TRUE>>=
plot(myContrasts[[1]], levelNames = paste("election", 1:4),
...
locating the MLE (for example see \texttt{help(House2001)}, where
...
unidiff <- gnm(Freq ~ educ*orig + educ*dest +
               Mult(Exp(-1 + educ), -1 + orig:dest), 
               ofInterest = "Mult1.Factor1", family = poisson,
...
getContrasts(doubleUnidiff, rev(pickCoef(doubleUnidiff, "Mult1.Factor1")))
getContrasts(doubleUnidiff, rev(pickCoef(doubleUnidiff, "Mult2.Factor1")))
@ %def
...
\[
g(\mu_{rc}) d= \alpha_r + \beta_c + \sum_{k=1}^K
