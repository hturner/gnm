...
\item[\Robject{localDesignFunction}]
  ...
  local design matrix. If the plug-in function does not return a 'start'
  component, this function must also take the argument \Rfunarg{ind}, the index
  of a column to be returned rather than the full matrix.
\end{description}
...
\[
\log \mu_{ic} =  \beta_{0c} + \gamma_c\sum_r \boldsymbol{\beta}_{r}\boldsymbol{x}_{ir}.
\]
...
--------------------
    ...
    o	updated plot.gnm() to give same plots as plot.lm() in R >= 2.2.0.

    o	hatvalues.gnm(), cooks.distance.gnm() and plot.gnm() replaced with 
	methods for "glm" objects, in which hatvalues are set to NaN when 
	calculated as being within rounding error of 1.
\end{verbatimtab}
