...
\texttt{search()}), however \Robject{C} would only be added to the
model frame if it was passed to \Rfunction{Nonlin}.
...
This model can be applied to the \Robject{yaish} data set \citep{Yais98}, 
which is a
