...
There are several \Rclass{nonlin} functions included in the
...
be built up. For example, one way to specify a logistic predictor function
of the form
...
If
predictors are named, these names are used as a prefix for parameter labels
or as the parameter label itself in the single-parameter case.

The list returned by the \Rclass{nonlin} function should also include the
following
...
\item[\Robject{match}:] (if \Robject{call} is non-\Rcode{NULL}) a numeric index
    ...
    \Robject{call}.
\item[\Robject{start}:] an optional function which takes a named vector of parameters
...
idea of useful starting values, so we will specify the \Robject{start}
component as
\begin{Scode}
function(theta){
    theta[3] <- 1
...
of the special ``\Robject{...}'' argument. The use of \Rfunction{match.call} is
...
The \Robject{common} component specifies that homogeneous effects are to
be estimated
across all the specified factors. The term only depends on these factors,
but the
\Robject{term} function allows for the empty \Robject{varLabels} vector
that will be
passed to it, by having a ``\Robject{...}'' argument.

Since the user may wish to specify multiple instances, a \Robject{call}
component
...
expressions passed to ``\Robject{...}'' may only represent single factors,
rather than
...
plot the distribution of Pearson residuals in Figure \ref{LCresplot}:
\begin{Sinput}
par(mfrow = c(2,2))
...
\begin{figure}[!tbph]
\begin{center}
\includegraphics[width=6in]{LCall.pdf}
\caption{Canada, males: plots of residuals from the Lee-Carter model
of mortality}
\end{center}
\label{LCresplot}
\end{figure}
Panel (a) of Figure \ref{LCresplot} indicates that the overdispersion
...
the distributions of Pearson residuals (Figure \ref{LCresplot2}).
\begin{figure}[!tbph]
\begin{center}
\includegraphics[width=6in]{LCover45.pdf}
...
ages
(Figure \ref{deaths2140}: the ages concerned are highlighted in solid red).
...
\begin{figure}[!tbph]
\begin{center}
\includegraphics{deaths1921-1940.pdf}
