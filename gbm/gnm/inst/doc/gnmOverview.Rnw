...
to \Rfunction{Nonlin}: a call to the relevant plug-in function and if
necessary, a
\Rclass{data.frame} object containing any variables that are required by
specified arguments of
...
score, FCFF) and a diagonal reference term with constant weights based on the two
