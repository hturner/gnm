...
system.time(temp.gnm <- gnm(counts ~ resp + resp:x, eliminate = rowID, family = poisson))
...
\begin{center}
<<profilePlot, fig = TRUE, echo = FALSE>>=
plot(prof)
title(list("Profile traces for the multipliers of the orig:dest association",
           cex = 0.9))
@ %def
\end{center}
...
<<Get_contrasts_1>>=
myContrasts <- getContrasts(doubleUnidiff, pickCoef(doubleUnidiff, "Mult1.Factor1")
myContrasts
@ %def
