...
for a square contingency table.  \citet{Agre02}, page ??, gives data on
...
of diagonal and symmetric interaction to cover situations with more than two
homologous factors.
...
factor unless otherwise specified. For example, to fit the row-column
association model
\[
...
\Robject{1} by default. This argument determines the number of multiplicative
...
\]
Exponentiated factors can be specified in \Rclass{gnm} models using the symbolic
function \Rfunction{Exp}, for example the uniform difference model above
would be specified by the formula
\begin{alltt}
    mu \twiddle\ R:T + C:T + Mult(Exp(-1 + T), R:C, multiplicity = 2)
\end{alltt}
...
The two plug-in functions included in the \Rpackage{gnm} package are described
...
equal across the factors. The arguments of Rfunction{MultHomog}
are the factors in
the interaction, which are assumed to be objects of class \Rclass{factor}.
Like a
\Rfunction{Mult} term, the interaction can include any number of factors,
but there
...
\citet{Agre02} page ??, which is a two-way contingency table classified by the child's
...
interaction. The singular value corresponding to each component is often
...
to a model with the first $n$ components of the multiplicative interaction.
...
grown at nine sites \citep{Wedd74,Gabr98}. The biplot model is fitted as follows
@
<<Biplot_model>>=
data(barley)
set.seed(1)
...
vectors can then be obtained as below
@
...
@ %Def
\end{center}
for sites $A \ldots I$ and varieties $1 \dots 9, X$. The product of the matrices
...
\begin{table}[!h]
\begin{tabular*}{\textwidth}{@{}p{0.2in}p{1in}p{4in}@{}}
    \toprule
    ...
    &	\Rfunction{getContrasts}	&  estimate contrasts and their
                                           standard errors \\
    &	\Rfunction{checkEstimable}\quad	&  check whether one or more parameter
                                           combinations in a
                                           \Rclass{gnm} model is identified \\
    &	\Rfunction{se}	&  get standard errors of linear parameter
                           combinations in \Rclass{gnm} models	\\
    &	\Rfunction{termPredictors}\quad	&  (\emph{generic}) extract term
