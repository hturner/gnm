...
We begin below with a preliminary note (Section \ref{glms}) on some
ways in which
...
In Section \ref{nonlinear} we shall see how nonlinear terms may be specified
...
to \Rclass{nonlin} functions. There are several \Rclass{nonlin} functions in the
\Rpackage{gnm} package. Some of these specify simple mathematical functions of
predictors: \Rfunction{Exp}, \Rfunction{Log}, \Rfunction{Mult}, \Rfunction{Inv},
\Rfunction{Raise} (to raise to a constant power), and \Rfunction{Logit}. Others
specify more specialised nonlinear terms, in particular \Rfunction{MultHomog}
...
In previous versions of \Rpackage{gnm}, specialised nonlinear terms
were implemented using plug-in functions and users could define custom plug-in
functions as described in Section \ref{plug-ins}. Such functions still work in
...
\begin{description}
\item[Exp] the exponential of a predictor
\item[Inv] the reciprocal of a predictor
\item[Log] the natural logarithm of a predictor
\item[Logit] the logit of a predictor
\item[Mult] the product of predictors
\item[Raise] a predictor raised to a constant power
\end{description}
...
be built up. For example, one way to specify a logistic term of the form
\[\frac{\text{Asym}}{1 + \exp((\text{xmid} - x)/\text{scal})}\]
would be as follows
\begin{Scode}
~ -1 + Mult(1, Inv(Const(1) + Exp(Mult(1 + offset(-log(conc)), Inv(1)))))
\end{Scode}
However, this is rather convoluted and it may be preferable to define a
specialised \Rclass{nonlin} function in such a case. Section
\ref{nonlin.functions} explains how users can define custom \Rclass{nonlin}
...
\]
also known as the Goodman RC model \citep{Good79}, would be specified by the
following formula
\begin{Scode}
mu ~ R + C + Mult(R, C)
\end{Scode}
...
\]
which would be represented by the following formula
\begin{Scode}
mu ~ R:T + C:T + Mult(Exp(T), R:C)
\end{Scode}

\subsection{\Rfunction{MultHomog}}

\Rfunction{MultHomog} is \ \Rclass{nonlin} function to specify
...
which is a formula with no left-hand side, specifying the dependence of
$\delta_f$ on any covariates. 
For example, the formula
\begin{Scode}
mu ~ -1 + x + Dref(R, C, delta = ~ 1 + x)
\end{Scode}
...
the model. For example, Goodman's RC model, introduced in section \ref{Basic}
\[
\log \mu_{rc} = \alpha_r + \beta_c + \gamma_r\delta_c,
\]
...
\end{description}
Intercepts will be added by default to predictors that are specified by formulae. If
predictors are named, these names will be used as a prefix for parameter labels
or the parameter label itself in the single parameter case.  

The list returned by the \Rclass{nonlin} function should also include the following
...
\end{description}
and may include any of the following components:
\begin{description}
...
As an example, consider the following logistic term
\[\frac{Asym}{1 + \exp((xmid - x)/scal)}\]

We can consider Asym, xmid and scal as the parameters of three separate predictors,
each with a single intercept term. Thus our \Robject{predictors} component would be
\begin{Scode}
...
argument named \Rfunarg{x}. Then our \Rclass{nonlin} function would return the
following \Robject{variables} component
\begin{Scode}
list(substitute(x))
\end{Scode}
...
Now we need to specify a function that will paste together an expression for
the term, given labels for the predictors and the variables:
\begin{Scode}
...
\end{Scode}
This would be the \Robject{term} component returned by our \Rclass{nonlin} function.

We now have all the necessary ingredients of a \Rclass{nonlin} function to specify
the logistic term. Since the parameterisation does not depend on user-specified
values, it does not make sense to use call-matched labelling in this case. The
labels for our parameters will be specified by the labels of the
\Robject{predictors} component. Since we do not anticipate fitting models with
multiple logistic terms, our \Rclass{nonlin} will not return a \Robject{call}
component with which to prefix the parameter labels. We do however, have some
idea of useful starting values, so we will specify the following \Robject{start} component
\begin{Scode}
    function(theta){
      theta[3] <- 1
      theta
...
The \Rfunction{MultHomog} function provides a further example a \Rclass{nonlin}
...
of the special ``...'' argument. The use of \Rfunction{match.call} is 
...
The \Robject{common} component specifies that homogeneous effects are to be estimated
across all the specified factors. The term only depend on these factors, but the
\Robject{term} function allows for the empty \Robject{varLabels} vector that will be
passed to it, by having a ``...'' argument. 

Since the user may wish to specify multiple instances, a \Robject{call} component
is returned, so that parameters in different instances of the term will have
unique labels (due to the \Rfunarg{inst} argument in the call). However as the
expressions passed to ``...'' may only represent single factors, rather than
general predictors, it is not necessary to use call-matched labelling, so a
vector of zeros is returned as the \Robject{match} component.
...
Section \ref{ofInterest} for further details on the use of the
...
substantive interest.  For an application see Section \ref{Stereotype} below.
...
the predictor.  See Section \ref{termPredictors} for an example of its use.

Most of the functions listed in Figure \ref{glm.lm} can be used as they would be
...
to estimable parameters. The deviance function of a generalised nonlinear model
...
\begin{center}
\includegraphics{gnmOverview-profilePlot.pdf}
\end{center}
...
Section~\ref{eliminate}, where a factor is required in the model in order
to represent
...
\ref{GAMMI}), or it may be crucial to success in
...
of the enlarged model, as can be seen in the following plot:
\begin{center}
<<residSVDplot, fig = TRUE, echo = FALSE>>=
plot(coef(biplotModel), biplotStart,
main = "Comparison of residSVD and MLE
for a 2-dimensional biplot model")
@ %def
\end{center}
...
\]
See Section \ref{Dref function} for more detail on the parameterization.
...
\Rfunction{residSVD} (see Section \ref{residSVD}) for the parameters
