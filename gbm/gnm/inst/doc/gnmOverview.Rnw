...
This document provides an extended overview
...
\Robject{C} has been passed to the
...
To fit this model, the formula argument to \Rfunction{gnm} would be
\begin{alltt}
...
\begin{figure}[!tbph]
    ...
    \caption{Generic functions in the \Rpackage{base}, \Rpackage{stats} and
      ...
      \Rclass{glm} or \Rclass{lm} objects, but which are not implemented for
      \Rclass{gnm} objects.}
    \label{!glm.lm}
\end{figure}

In addition to the accessor functions shown in Figure \ref{glm.lm}, the
\Rpackage{gnm} package provides a new generic function \Rfunction{termPredictors}
...
grown at nine sites. The biplot model is fitted as follows
@
<<Biplot_model>>=
data(barley)
set.seed(1)
...
B
@ %def
These matrices are essentially the same as in \citet{Gabr98}, from which the
...
The stereotype model was proposed by \citep{Ande84} for ordered categorical
...
\[
\log \mu_c =  \beta_{0c} + \gamma_c\sum_i \beta_{ic}x_i.
\]
