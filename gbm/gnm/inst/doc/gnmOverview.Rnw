...
the same form as that used by \Rfunction{SSlogis} (a
  selfStart model for use with \Rfunction{nls} --- see \ref{sec:gnmVnls} for
...
plot(coef(biplotModel), biplotStart, 
     main = "Comparison of residSVD and MLE for a 2-dimensional biplot model")
@ %def 
...
\section{\Rfunction{gnm} or \Rfunction{nls}?}
\label{sec:gnmVnls}
...
\Rfunction{gnm} is to \Rfunction{nls} what \Rfunction{glm} is to \Rfunction{lm},
in that a \Rfunction{nls} model is equivalent to a \Rfunction{gnm} model with
\Rcode{family = gaussian}. Whilst \Rfunction{nls} assumes that the responses are
distributed with common variance or fixed relative variances, \Rfunction{gnm}
allows for responses distributed with variances that are a common function of
the mean. 

In terms of the interface, however, \Rfunction{gnm} and \Rfunction{nls} are
quite different. Models are specified to \Rfunction{nls} in terms of a
mathematical formula, which is convenient for models that have a small number
of parameters. For models that have a large number of parameters, or can not
easily be represented by a mathematical formula, the symbolic model
specification used by \Rfunction{gnm} may be more convenient. This would usually
be the case for models involving factors, which would need to be represented by
dummy variables in a \Rfunction{nls} formula. 

When working with artificial data, \Rfunction{gnm} has a minor advantage over
\Rfunction{nls} in that it does not fail when a model is an exact fit to the
data. Therefore it is not necessary to add noise to artificial data, which can
