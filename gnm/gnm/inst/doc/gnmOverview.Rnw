...
Prior to the introduction (in \Rpackage{gnm} version 0.9-1) of \Rclass{nonlin} functions, nonlinear terms that
...
\Rclass{nonlin} functions, but user-specified plug-in functions may still be
