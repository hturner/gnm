...
constrained via \Rfunarg{constrain} and \Rfunarg{constrainTo}
arguments. Parameters of a stratification factor can be
handled more efficiently by specifying the factor in an \Rfunarg{eliminate}
argument. These options are described in more detail below.
...
\Rclass{nonlin} functions or plug-in functions.
\item
\label{i:start}
...
\Rfunarg{constrain} or a plug-in function.
...
\end{Soutput}
see Section \ref{sec:getContrasts} for further details of this function.
...
set as the reference level; alternatives may be set via the \Rfunarg{refLevel}
argument.

In the above example, the simple contrasts are estimable without scaling. This
is not always the case and a more general family of contrasts for a set of
parameters $\gamma_r, r = 1, \ldots, R$ is given by
\begin{equation*}
    \gamma^*_r = \frac{\gamma_r - \overline{gamma}_w}{
      \sqrt{\sum v_r * (\gamma_r - \overline{\gamma_w})^2}
\end{equation*}
where $\overline{gamma}_w = \sum w_r \gamma_r$, the reference level against
which the contrasts are taken, and $v_r$ is a set of weights which may be
specified to \Rfunction{getContrasts} via the \Rfunarg{scaleWeights}
argument. Equal scale weights lead to spherical contrasts, whilst unequal
weights lead to elliptical contrasts; examples of these cases are given in
Section \ref{sec:GAMMI} and Section \ref{sec:RCmodels} respectively.
...
mu <- getContrasts(RC1model, pickCoef(RC1model, "[.]SES"),
                   refLevel = rowProbs, scaleWeights = rowProbs)
nu <- getContrasts(RC1model, pickCoef(RC1model, "[.]MHS"),
                   refLevel = colProbs, scaleWeights = colProbs)
mu
nu
@ %def
Since the value of \Robject{beta} is dependent on the scaling of the contrasts,
it is not advisable to conduct inference on this parameter directly. However the
...
@
<<Spherical_contrasts>>=
gammi <- getContrasts(barleyModel, pickCoef(barleyModel, "[.]y"),
                      refLevel = "mean", scale = "unit")
delta <- getContrasts(barleyModel, pickCoef(barleyModel, "[.]g"),
                      refLevel = "mean", scale = "unit")
gammi
delta
@ %def
On the basis of this information we can investigate simplifications of the model
in which levels of the factors are combined or an additive model is fitted to a
subset of the data.
