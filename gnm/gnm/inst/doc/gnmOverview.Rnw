...
\begin{equation*}
    \gamma^*_r = \frac{\gamma_r - \overline{\gamma}_w}{
      \sqrt{\sum v_r * (\gamma_r - \overline{\gamma}_u)^2}}
\end{equation*}
where $\overline{gamma}_w = \sum w_r \gamma_r$ is the reference level against
which the contrasts are taken; $\overline{gamma}_u$ is a possibly different
weighted mean of the parameters and $v_r$ is a set of weights. This type of
scaling can be obtained by specifying the form of $\overline{gamma}_u$ and $v_r$
to the \Rfunarg{scaleRef} and \Rfunarg{scaleWeights} arguments of
...
identified when both their scale and location is constrained. A simple way to
achieve this is to set the first effect to zero and the last effect to one:
@
<<RCmodel_constrained>>=
...
to maintain equivalence with the original parameterization. These constraints
are equivalent to scaled contrasts with the first effect, in which the scaling
also takes the first effect as the reference level and the scaling weights are
all zero except the last:
@
<<getContrasts_simple>>=
...
contrasts, whilst unequal weights produce elliptical contrasts. Further examples
