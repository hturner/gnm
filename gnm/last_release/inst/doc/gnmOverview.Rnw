...
first used to fit a model with only the linear terms (offsetting any terms that
are constrained). In this case the parameters corresponding to the predictor
...
variable in a nonlinear least squares model with only the nonlinear terms,
offsetting the fitted linear predictors. Since the model is over-parameterized,
...
\begin{figure}[!tbph]
    ...
    \caption{Generic functions in the \Rpackage{base}, \Rpackage{stats} and
      ...
      \Rclass{gnm} objects. Starred functions are implemented for models with linear terms only.}
    \label{fig:glm.lm}
\end{figure}
...
\end{align*}
and the variable effects can be similarly represented.

Thus we can represent the sites and varieties separately as follows
@
<<Effect_plots, fig = TRUE, include = FALSE, width=10, height=5>>=
...
\begin{figure}[!tbph]
    \begin{center}
        \includegraphics{gnmOverview-Effect_plots.pdf}
    \end{center}
...
text(c(-3.5, -3.5), c(3.9, 3.6), c("sites: A-I","varieties: 1-9, X"), adj = 0)
@ %def
\begin{figure}[!tbph]
    \begin{center}
...
the sterotype model can be estimated through fitting the following model
\begin{align*}
...
grouped together, to avoid redundacy.
...
logLikMultinom <- function(model, size){
    ...
    c(nParameters = object$rank - object$eliminate, logLikelihood = l)
}
...
@ %def
In this instance, only 36 of the 100 calls to \Rfunction{gnm} have successfully
located a local maximum of the likelihood: in the remaining 64 cases the
...
the last of these, the exact fit to the data, having been found 11 times
out of the above 36).
