# some tests from tests/hskewL.R in logmult package
...
test_that("gnmFit handles computationally singular initial linear fit ", {
    if (requireNamespace("logmult")){
        # EGP class of cohabiting spouses where one is 30-60 
        # (last occupation for inactive persons)
        # French Labour Force Surveys, 1969 and 2011
        tab2 <- structure(
            c(261L, 43L, 21L, 5L, 7L, 26L, 5L, 16L, 17L, 7L, 1L,
        ...
        expect_equal(round(c(model2$assoc$row), 2),
                     c(1.97, 1.38,  0.05, -0.24, -1.02, 0.57, -0.79, -0.16,
                       -0.14, -1.32, -1.56,
                       0,   -0.03, -0.94,  0.98, -0.10, 0.71,  2.65, -0.86,
                       -0.66, -0.77,  0.79))
    }
})